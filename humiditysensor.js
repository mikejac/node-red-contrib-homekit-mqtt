...
module.exports = function (RED) {
    ...
    function HAPHumiditySensorNode(config) {
        ...
        if (this.clientConn) {
            ...
            RED.log.error("HAPHumiditySensorNode(): no clientConn")
        }
        ...
        this.clientConn.updateSubscribe(this.nodename, this.dataId, this.qos, function(topic, payload, packet) {
            RED.log.debug("HAPHumiditySensorNode(updateSubscribe): payload = " + payload.toString())
            node.clientConn.startAliveTimer(node)
            ...
            try {
                ...
                if (obj.hasOwnProperty('currentrelativehumidity')) {
                    ...
                    RED.log.debug("HAPHumiditySensorNode(updateSubscribe): currentrelativehumidity = " + obj.currentrelativehumidity)
                }
                if (obj.hasOwnProperty('statusactive')) {
                    service.setCharacteristic(Characteristic["StatusActive"], obj.statusactive)
                    RED.log.debug("HAPHumiditySensorNode(updateSubscribe): statusactive = " + obj.statusactive)
                }
                if (obj.hasOwnProperty('statusfault')) {
                    service.setCharacteristic(Characteristic["StatusFault"], obj.statusfault)
                    RED.log.debug("HAPHumiditySensorNode(updateSubscribe): statusfault = " + obj.statusfault)
                }
                if (obj.hasOwnProperty('statustampered')) {
                    service.setCharacteristic(Characteristic["StatusTampered"], obj.statustampered)
                    RED.log.debug("HAPHumiditySensorNode(updateSubscribe): statustampered = " + obj.statustampered)
                }
                if (obj.hasOwnProperty('statuslowbattery')) {
                    service.setCharacteristic(Characteristic["StatusLowBattery"], obj.statuslowbattery)
                    RED.log.debug("HAPHumiditySensorNode(updateSubscribe): statuslowbattery = " + obj.statuslowbattery)
                }
            } catch(err) {
        ...
        service.on('characteristic-change', function (info) {
            ...
            RED.log.debug("HAPHumiditySensorNode(characteristic-change): key = " + key + ", value = " + info.newValue)
            ...
            switch (key) {
                case "CurrentRelativeHumidity":
                    RED.log.debug("HAPHumiditySensorNode(characteristic-change): CurrentRelativeHumidity")
                    break
                case "StatusActive":
                    RED.log.debug("HAPHumiditySensorNode(characteristic-change): StatusActive")
                    break
                case "StatusFault":
                    RED.log.debug("HAPHumiditySensorNode(characteristic-change): StatusFault")
                    break
                case "StatusTampered":
                    RED.log.debug("HAPHumiditySensorNode(characteristic-change): StatusTampered")
                    break
                case "StatusLowBattery":
                    RED.log.debug("HAPHumiditySensorNode(characteristic-change): StatusLowBattery")
                    break
                default:
        ...
        this.rpcReply = function(reply) {
            RED.log.debug("HAPHumiditySensorNode(rpcReply)" + JSON.stringify(reply))
            node.clientConn.startAliveTimer(node)
        }
        ...
        this.online = function(status) {
            RED.log.debug("HAPHumiditySensorNode(online): " + status)
        ...
        this.on('input', function (msg) {
            ...
            if (!msg.hasOwnProperty('topic')) {
                ...
                //
                if (msg.topic.toUpperCase() == "CURRENTRELATIVEHUMIDITY") {
                    ...
                    if (msg.payload.hasOwnProperty('currentrelativehumidity')) {
                        RED.log.debug("HAPHumiditySensorNode(input): currentrelativehumidity")
                    ...
                    if (msg.payload.hasOwnProperty('statuslowbattery')) {
                        RED.log.debug("HAPHumiditySensorNode(input): statuslowbattery")
    ...
    RED.nodes.registerType('homekit-humiditysensor-v2', HAPHumiditySensorNode)
}
