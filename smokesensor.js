...
module.exports = function (RED) {
    ...
    function HAPSmokeSensorNode(config) {
        ...
        try {
            ...
            RED.log.debug("HAPSmokeSensorNode(): service already exists")
        ...
        if (this.clientConn) {
            ...
            RED.log.error("HAPSmokeSensorNode(): no clientConn")
        }
        ...
        this.clientConn.updateSubscribe(this.nodename, this.dataId, this.qos, function(topic, payload, packet) {
            RED.log.debug("HAPSmokeSensorNode(updateSubscribe): payload = " + payload.toString())
            node.clientConn.startAliveTimer(node)

            try {
                var obj = JSON.parse(payload)

                if (obj.hasOwnProperty('smokedetected')) {
                    service.setCharacteristic(Characteristic["SmokeDetected"], obj.smokedetected)
                    RED.log.debug("HAPSmokeSensorNode(updateSubscribe): smokedetected = " + obj.smokedetected)
                }
                if (obj.hasOwnProperty('statusactive')) {
                    service.setCharacteristic(Characteristic["StatusActive"], obj.statusactive)
                    RED.log.debug("HAPSmokeSensorNode(updateSubscribe): statusactive = " + obj.statusactive)
                }
                if (obj.hasOwnProperty('statusfault')) {
                    service.setCharacteristic(Characteristic["StatusFault"], obj.statusfault)
                    RED.log.debug("HAPSmokeSensorNode(updateSubscribe): statusfault = " + obj.statusfault)
                }
                if (obj.hasOwnProperty('statustampered')) {
                    service.setCharacteristic(Characteristic["StatusTampered"], obj.statustampered)
                    RED.log.debug("HAPSmokeSensorNode(updateSubscribe): statustampered = " + obj.statustampered)
                }
                if (obj.hasOwnProperty('statuslowbattery')) {
                    service.setCharacteristic(Characteristic["StatusLowBattery"], obj.statuslowbattery)
                    RED.log.debug("HAPSmokeSensorNode(updateSubscribe): statuslowbattery = " + obj.statuslowbattery)
                }
            } catch(err) {
        ...
        service.on('characteristic-change', function (info) {
            ...
            RED.log.debug("HAPSmokeSensorNode(characteristic-change): key = " + key + ", value = " + info.newValue)
            ...
            switch (key) {
                case "SmokeDetected":
                    RED.log.debug("HAPSmokeSensorNode(characteristic-change): SmokeDetected")
                    break
                case "StatusActive":
                    RED.log.debug("HAPSmokeSensorNode(characteristic-change): StatusActive")
                    break
                case "StatusFault":
                    RED.log.debug("HAPSmokeSensorNode(characteristic-change): StatusFault")
                    break
                case "StatusTampered":
                    RED.log.debug("HAPSmokeSensorNode(characteristic-change): StatusTampered")
                    break
                case "StatusLowBattery":
                    RED.log.debug("HAPSmokeSensorNode(characteristic-change): StatusLowBattery")
                    break
                default:
        ...
        this.rpcReply = function(reply) {
            RED.log.debug("HAPSmokeSensorNode(rpcReply)" + JSON.stringify(reply))
            node.clientConn.startAliveTimer(node)
        }
        ...
        this.online = function(status) {
            RED.log.debug("HAPSmokeSensorNode(online): " + status)
        ...
        this.on('input', function (msg) {
            ...
            if (!msg.hasOwnProperty('topic')) {
                ...
                //
                if (msg.topic.toUpperCase() == "SMOKEDETECTED") {
                    ...
                    if (msg.payload.hasOwnProperty('Smokedetected')) {
                        RED.log.debug("HAPSmokeSensorNode(input): smokedetected")
                        service.setCharacteristic(Characteristic["SmokeDetected"], msg.payload.smokedetected)
                    }
                    if (msg.payload.hasOwnProperty('statusactive')) {
                        RED.log.debug("HAPSmokeSensorNode(input): statusactive")
                        service.setCharacteristic(Characteristic["StatusActive"], msg.payload.statusactive)
                    }
                    if (msg.payload.hasOwnProperty('statusfault')) {
                        RED.log.debug("HAPSmokeSensorNode(input): statusfault")
                        service.setCharacteristic(Characteristic["StatusFault"], msg.payload.statusfault)
                    }
                    if (msg.payload.hasOwnProperty('statustampered')) {
                        RED.log.debug("HAPSmokeSensorNode(input): statustampered")
                    ...
                    if (msg.payload.hasOwnProperty('statuslowbattery')) {
                        RED.log.debug("HAPSmokeSensorNode(input): statuslowbattery")
    ...
    RED.nodes.registerType('homekit-smokesensor-v2', HAPSmokeSensorNode)
}
