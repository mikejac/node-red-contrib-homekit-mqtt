...
module.exports = function (RED) {
    ...
    function HAPOccupancySensorNode(config) {
        ...
        try {
            ...
            RED.log.debug("HAPOccupancySensorNode(): service already exists")
        ...
        if (this.clientConn) {
            ...
            RED.log.error("HAPOccupancySensorNode(): no clientConn")
        }
        ...
        this.clientConn.updateSubscribe(this.nodename, this.dataId, this.qos, function(topic, payload, packet) {
            RED.log.debug("HAPOccupancySensorNode(updateSubscribe): payload = " + payload.toString())
            node.clientConn.startAliveTimer(node)

            try {
                var obj = JSON.parse(payload)

                if (obj.hasOwnProperty('occupancydetected')) {
                    service.setCharacteristic(Characteristic["OccupancyDetected"], obj.occupancydetected)
                    RED.log.debug("HAPOccupancySensorNode(updateSubscribe): occupancydetected = " + obj.occupancydetected)
                }
                if (obj.hasOwnProperty('statusactive')) {
                    service.setCharacteristic(Characteristic["StatusActive"], obj.statusactive)
                    RED.log.debug("HAPOccupancySensorNode(updateSubscribe): statusactive = " + obj.statusactive)
                }
                if (obj.hasOwnProperty('statusfault')) {
                    service.setCharacteristic(Characteristic["StatusFault"], obj.statusfault)
                    RED.log.debug("HAPOccupancySensorNode(updateSubscribe): statusfault = " + obj.statusfault)
                }
                if (obj.hasOwnProperty('statustampered')) {
                    service.setCharacteristic(Characteristic["StatusTampered"], obj.statustampered)
                    RED.log.debug("HAPOccupancySensorNode(updateSubscribe): statustampered = " + obj.statustampered)
                }
                if (obj.hasOwnProperty('statuslowbattery')) {
                    service.setCharacteristic(Characteristic["StatusLowBattery"], obj.statuslowbattery)
                    RED.log.debug("HAPOccupancySensorNode(updateSubscribe): statuslowbattery = " + obj.statuslowbattery)
                }
            } catch(err) {
        ...
        service.on('characteristic-change', function (info) {
            ...
            RED.log.debug("HAPOccupancySensorNode(characteristic-change): key = " + key + ", value = " + info.newValue)
            ...
            switch (key) {
                case "OccupancyDetected":
                    RED.log.debug("HAPOccupancySensorNode(characteristic-change): OccupancyDetected")
                    break
                case "StatusActive":
                    RED.log.debug("HAPOccupancySensorNode(characteristic-change): StatusActive")
                    break
                case "StatusFault":
                    RED.log.debug("HAPOccupancySensorNode(characteristic-change): StatusFault")
                    break
                case "StatusTampered":
                    RED.log.debug("HAPOccupancySensorNode(characteristic-change): StatusTampered")
                    break
                case "StatusLowBattery":
                    RED.log.debug("HAPOccupancySensorNode(characteristic-change): StatusLowBattery")
                    break
                default:
        ...
        this.rpcReply = function(reply) {
            RED.log.debug("HAPOccupancySensorNode(rpcReply)" + JSON.stringify(reply))
            node.clientConn.startAliveTimer(node)
        }
        ...
        this.online = function(status) {
            RED.log.debug("HAPOccupancySensorNode(online): " + status)
        ...
        this.on('input', function (msg) {
            ...
            if (!msg.hasOwnProperty('topic')) {
                ...
                //
                if (msg.topic.toUpperCase() == "OCCUPANCYDETECTED") {
                    ...
                    if (msg.payload.hasOwnProperty('occupancydetected')) {
                        RED.log.debug("HAPOccupancySensorNode(input): occupancydetected")
                        service.setCharacteristic(Characteristic["OccupancyDetected"], msg.payload.occupancydetected)
                    }
                    if (msg.payload.hasOwnProperty('statusactive')) {
                        RED.log.debug("HAPOccupancySensorNode(input): statusactive")
                        service.setCharacteristic(Characteristic["StatusActive"], msg.payload.statusactive)
                    }
                    if (msg.payload.hasOwnProperty('statusfault')) {
                        RED.log.debug("HAPOccupancySensorNode(input): statusfault")
                        service.setCharacteristic(Characteristic["StatusFault"], msg.payload.statusfault)
                    }
                    if (msg.payload.hasOwnProperty('statustampered')) {
                        RED.log.debug("HAPOccupancySensorNode(input): statustampered")
                    ...
                    if (msg.payload.hasOwnProperty('statuslowbattery')) {
                        RED.log.debug("HAPOccupancySensorNode(input): statuslowbattery")
    ...
    RED.nodes.registerType('homekit-occupancysensor-v2', HAPOccupancySensorNode)
}
